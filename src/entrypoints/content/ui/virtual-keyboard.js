...
export class VirtualKeyboard {
	...
	static create(input, parent, closeCallback) {
		...
		const keys = {};
		const appendKey = (text, widthPercent) => {
		...
		for (const label in LAYOUT_BOTTOM) {
	...
	alignWithParent() {
		...
		const targetBounds = this.parent.getBoundingClientRect();
		...
		const searchWidth = targetBounds.width - 2;
		...
		const keyboardWidth = parseFloat(
			window.getComputedStyle(this.keyboard).width,
		);
	...
	select(key) {
		if (this.selected) {
			this.keys[this.selected].classList.remove(
				"gamepad-interface-selected-key",
			);
		}
	...
	backspace() {
		if (this.input.value.length > 0) {
			const c = this.input.value[this.input.value.length - 1];
			this.input.value = this.input.value.substring(
				0,
				this.input.value.length - 1,
			);
			this.dispatchInputChangeEvent();
			const isRemovedUpper = c === c.toUpperCase() && c !== c.toLowerCase();
	...
	toggleShift(force) {
		...
		for (const key in this.keys) {
			if (key.length === 1) {
				const letter = this.keys[key].innerHTML;
				this.keys[key].innerHTML = this.shift
					? letter.toUpperCase()
					: letter.toLowerCase();
			}
		}
	...
	onDirectionAction(direction) {
		const key = this.getAdjacentKey(direction);
		if (key) {
			this.select(key);
		}
	}
	...
	getAdjacentKey(direction) {
					...
					switch (direction) {
						...
						case DIRECTION.DOWN:
							if (y < LAYOUT.length - 1) {
								...
								let cumulativeX = 0;
								for (const key in LAYOUT_BOTTOM) {
		...
		// bottom row keys
		let cumulativeX = 0;
		const bottomKeys = Object.keys(LAYOUT_BOTTOM);
		for (let x = 0; x < bottomKeys.length; x++) {
			if (bottomKeys[x] === this.selected) {
				switch (direction) {
					case DIRECTION.UP: {
						const upX =
							cumulativeX + Math.floor(LAYOUT_BOTTOM[bottomKeys[x]] / 2) - 1;
